...
void gen_data(float bounds[10], int index)
{
  ...
  OCP ocp( 0.0, 0.5 );
  // C3
  // ocp.minimizeMayerTerm((T*T + tau1*qd1*tau1*qd1 + tau2*qd2*tau2*qd2)*(1/(500*500-tau1*tau1))*(1/(500*500-tau2*tau2)));
  // C4
  ocp.minimizeMayerTerm((1/(500*500-tau1*tau1))*(1/(500*500-tau2*tau2)));
  ...
  float cost = algorithm.getObjectiveValue();
  ...
  if ((diff.getNorm(VN_L2) < 0.001)&&(cost>0.0)){
