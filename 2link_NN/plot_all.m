...
control_std = dlmread('trained_models/std_knn_control_100.txt');
control_scaled = dlmread('trained_models/scaled_knn_control_100.txt');
cost_std = dlmread('trained_models/std_knn_cost_100.txt');
cost_scaled = dlmread('trained_models/scaled_knn_cost_100.txt');
knn_error = dlmread('trained_models/knn_error_new.txt');
nn_error = dlmread('trained_models/nn_error_new.txt');
knn_descaled_error = dlmread('trained_models/knn_descaled_error_new.txt');
nn_descaled_error = dlmread('trained_models/nn_descaled_error_new.txt');

figure(1)
subplot(2,2,1)
plot(control_std(:,1),control_std(:,2));
xlim([3,15])
...
subplot(2,2,2)
plot(control_scaled(:,1),control_scaled(:,2));
xlim([3,15])
xlabel('Number of neighbours');
...
subplot(2,2,4)
plot(cost_scaled(:,1),cost_scaled(:,2));
xlim([3,15])
xlabel('Number of neighbours');
...
ylabel('Scaled torque')
legend('nn error')
title('Scaled NN error')

subplot(2,2,4)
plot(nn_descaled_error)
xlabel('time (\times 0.025)')
ylabel('Actual torque')
legend('nn descaled error')
title('Actual NN error')

figure(3)
% t=1:1:40;
yknn = ykNN((ykNN > -400) < 400)
ynn = yNN((yNN > -400) < 400)
plot(ykNN)
% hold on
% plot(ynn)
