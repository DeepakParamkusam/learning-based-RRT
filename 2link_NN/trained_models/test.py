...
def test_control_knn(num_data, type_data, num_neigh):
    data = '../training_data/control_ft_2_'+ str(num_data) + 'k_' + type_data + '_new'
    #load data
    X_train,Y_train,X_validate,Y_validate,coeff = joblib.load(data)
    knn = joblib.load('knn/control/knn_control_ft_2_' + str(num_data) + 'k_' + type_data + '_' + str(num_neigh)+'_new')
...
def main(flag, num_data = 100, type_data = 'scaled', num_neigh = 7, control_HL1 = 32, control_HL2 = 0, cost_HL1 = 32, cost_HL2 = 0):
    if flag == 0:
        ...
        nn = test_cost_nn(num_data,cost_HL1,cost_HL2)

    out_knn = knn.predict(X_train_knn[0])
    out_nn = nn.predict(X_train_nn[0].reshape([1,8]))

    numpy.savetxt('knn_error_new_100.txt',out_knn - Y_train_knn[0],delimiter=',')
    # plot.plot(out_knn - Y_train_knn[0])
    # plot.show()
    numpy.savetxt('knn_descaled_error_new_100.txt',(out_knn*coeff_knn[3] + coeff_knn[2]) - (Y_train_knn[0]*coeff_knn[3] + coeff_knn[2]),delimiter=',')
    numpy.savetxt('nn_error_new_100.txt',out_nn - Y_train_knn[0],delimiter=',')
    numpy.savetxt('nn_descaled_error_new_100.txt',(out_nn*coeff_nn[3] + coeff_nn[2]) - (Y_train_nn[0]*coeff_nn[3] + coeff_nn[2]),delimiter=',')
