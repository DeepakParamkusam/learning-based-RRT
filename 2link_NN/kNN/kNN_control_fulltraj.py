...
if len(sys.argv) == 4:
    ...
    data = '../training_data/final_data/final_control_ft_2_' + num_data + 'k_' + type_data + '_' + rep_parameter
    #load data
    X_train,Y_train,X_validate,Y_validate,coeff = joblib.load(data)
    #open file to save metrics
    to_file = open('../trained_models/' + type_data + '_knn_control_' + num_data + '_' + rep_parameter + '.txt', 'a')
    ...
    for num_neigh in range(5,10):
        ...
        #save kNN
        joblib.dump(knn, '../trained_models/knn/control/knn_control_ft_2_' + num_data + 'k_' + type_data + '_' + rep_parameter + '_' + str(num_neigh))
else:
